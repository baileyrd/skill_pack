...
#
# Surfaces candidate hits for one gap-analysis row across platform repos —
# used in step 1 to check whether a gap could be filled by porting an
# existing implementation instead of writing one from scratch. Each --repos
# entry can be a local path (used as-is) or an owner/repo slug
...
# a verdict.
#
# Same mechanics as sovereignty-loop's/dedupe-loop's/issue-loop's
# scan_platform_repos.sh, kept as its own copy so this skill stays
# self-contained.
#
