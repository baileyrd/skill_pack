...
writing new code from scratch, spanning **both** namespaces — the personal
...
in either place; treat this as one merged list, not two to check separately.
Confirm against `gh repo list Rusty-Mill` and `gh repo list baileyrd
--source` (or a manual name check) since both grow and repos move between
them over time — this file is a cache, not gospel.
...
| rusty_prime_agent (rusty_prime_agent rewrite) | baileyrd | Rust rewrite of Prime Agent's daemon/worker/session core |
| RustyMill (org umbrella) | Rusty-Mill | Not a repo to scan itself — the org home for the above |

The two standards/AKB repos are listed for completeness but are never reuse
candidates — they're documentation, not code; see
`references/development-standards.md` for how they're actually used.
...
name match isn't the same as coverage, and this is a candidate list to
judge, not a verdict.
...
to build the slug (`Rusty-Mill/rusty_json`, `baileyrd/rusty_prime_agent`) —
don't assume everything is still under the personal namespace or that
migration to `Rusty-Mill` is finished for a given repo; check this table
first, then fall back to trying both namespaces if a repo isn't listed here
yet.
