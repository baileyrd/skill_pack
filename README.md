...
| [`repo-inspector/`](my_loops/repo-inspector) | Dry-run only: ports `dedupe-loop`'s clustering/classification and `sovereignty-loop`'s dependency detection to work *within* one Cargo workspace (the RustyMill monorepo) instead of across repos. Produces one `repo-inspector-report.md` — duplication clusters plus sovereignty findings — with no issues, PRs, code changes, or auto-merge; every row is left for human review. |
| [`implementation-merge/`](my_loops/implementation-merge) | Picks up where `dedupe-loop`/`repo-inspector`'s `convergent-but-diverged` classification stops: given 2+ already-identified candidate implementations, classifies mergeability, then proposes an actual merged implementation combining the best of each — dry-run only, a `MERGE-PROPOSAL.md` plus scratch source, nothing written into a candidate's real path. Never silently drops an item from a losing candidate. |
...
| [`repo-config/`](my_loops/repo-config) | Scans a repo and applies the standard governance file set (PR/issue templates, README, CONTRIBUTING, SECURITY, CHANGELOG, ARCHITECTURE, ADR seed, `.gitattributes`). |
