...
## Structure
Three category folders (`my_loops/`, `yt_research_for_cc/`, `meta/`), each holding independently-versioned skill directories, plus repo-wide tooling under `scripts/`. `meta/` is the odd one out deliberately: its skills (`skill-retro`, `learn-it`) target this repo's own skill directories rather than an external repo, but they're still independently-versioned, PR-gated skill directories like every other category — the category boundary is about *subject matter* (an external target repo vs. this repo's own skills), not a different governance model. There is no modular-monolith-vs-service question to answer yet — see Boundaries above for why the generic architecture default doesn't apply as written here; it would apply the moment two skills need to share code across a real dependency edge, at which point `ATLAS-100` becomes the reference to consult, not this repo's own opinion.
