...
| --- | --- | --- |
| `git remote get-url origin` | git | `{{OWNER_REPO}}` |
...
| Architecture default | Modular monolith. Extract a separate service only for a concrete forcing function — independent scaling, a team/language boundary, or hard fault isolation. |
| Boundary pattern | Ports-and-adapters — domain logic stays free of I/O and framework details. |
| `{{OWNER_REPO}}` | `<fill in once a git remote is set>` |
| `{{SECURITY_CONTACT}}` | `<fill in — team alias or individual>` |
...
- Security contact — never inferable from the repo itself
